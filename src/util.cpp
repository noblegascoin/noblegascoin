...
#else
    const char* pszModule = "noblegascoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NobleGasCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NobleGasCoin";
#else
    // Unix
    return pathRet / ".noblegascoin";
#endif
#endif
}
